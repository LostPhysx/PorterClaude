...
// container (recipe and custom alike) mounts the host's tools volume read-only and gets the
...
export const RECIPES: RecipeDef[] = [
  ...
  { name: 'php', title: 'PHP 8.3 + nginx', description: 'php:8.3-fpm with nginx, composer and supervisord', baseImage: 'php:8.3-fpm-bookworm', defaultPorts: [80], notes: 'serves /workspace/public on port 80; set the container env var PC_HTTP_PORT (and publish the same port) when the host does not allow uid 1000 to bind port 80' },
